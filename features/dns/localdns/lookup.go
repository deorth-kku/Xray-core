...
func (c *Client) QueryIP(ctx context.Context, domain string, option dns.IPOption) ([]net.IP, uint32, error) {
	...
	ips, err := c.r.LookupIP(ctx, network, domain)
	if err != nil {
		return nil, 0, err
	}
	if len(ips) == 0 {
...
func (c *Client) LookupTXT(ctx context.Context, name string) ([]string, error) {
	return c.r.LookupTXT(ctx, name)
}

func (c *Client) LookupSRV(ctx context.Context, service string, proto string, name string) (string, []*gonet.SRV, error) {
	return c.r.LookupSRV(ctx, service, proto, name)
}
