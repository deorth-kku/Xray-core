		...
		if rec := cache.findRecords(fqdn); rec != nil {
			...
			if !go_errors.Is(err, errRecordNotFound) {
				if ttl > 0 {
					...
					errors.Log(ctx, &log.DNSLog{Server: cache.name, Domain: fqdn, Result: ips, Status: log.DNSCacheHit, Elapsed: 0, Error: err})
				...
				if cache.serveStale && (cache.serveExpiredTTL == 0 || cache.serveExpiredTTL < ttl) {
					...
					errors.Log(ctx, &log.DNSLog{Server: cache.name, Domain: fqdn, Result: ips, Status: log.DNSCacheOptimiste, Elapsed: 0, Error: err})
...
func doFetch(ctx context.Context, s CachedNameserver, fqdn string, option dns.IPOption) result {
	...
	errors.Log(ctx, &log.DNSLog{Server: s.getCacheController().name, Domain: fqdn, Result: ips, Status: log.DNSQueried, Elapsed: time.Since(start), Error: err})
