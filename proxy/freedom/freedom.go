...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	err := retry.ExponentialBackoff(5, 100).On(func() error {
		...
		if h.config.DomainStrategy.HasStrategy() && dialDest.Address.Family().IsDomain() {
			...
			ips, err := internet.LookupForIP(ctx, dialDest.Address.Domain(), strategy, outGateway)
			if err != nil {
	...
	requestDone := func() error {
		...
		if destination.Network == net.Network_TCP {
			...
			writer = NewPacketWriter(ctx, conn, h, UDPOverride, destination)
...
func NewPacketWriter(ctx context.Context, conn net.Conn, h *Handler, UDPOverride net.Destination, DialDest net.Destination) buf.Writer {
	iConn := conn
	statConn, ok := iConn.(*stat.CounterConnection)
	if ok {
		iConn = statConn.Connection
			...
			if b.UDP.Address.Family().IsDomain() {
				if ip, ok := w.ResolvedUDPAddr.Load(b.UDP.Address.Domain()); ok {
					...
					if w.Handler.config.DomainStrategy.HasStrategy() {
						ips, err := internet.LookupForIP(w.ctx, b.UDP.Address.Domain(), w.Handler.config.DomainStrategy, w.LocalAddr)
						if err != nil {
